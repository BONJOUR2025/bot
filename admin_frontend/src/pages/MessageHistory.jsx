...
const summaryDescriptors = [
  { key: 'total', label: 'Всего', color: 'bg-slate-100 text-slate-700' },
  { key: 'success', label: 'Успешно', color: 'bg-green-100 text-green-700' },
  { key: 'pending', label: 'В ожидании', color: 'bg-yellow-100 text-yellow-700' },
  { key: 'errors', label: 'Ошибки', color: 'bg-red-100 text-red-700' },
  { key: 'invalid', label: 'Невалидные', color: 'bg-orange-100 text-orange-700' },
  { key: 'other', label: 'Прочие', color: 'bg-gray-200 text-gray-700' },
];
...
export default function MessageHistory() {
  ...
  useEffect(() => {
    const firstProblematic = entries.find(
      (entry) =>
        entry.broadcast &&
        entry.summary &&
        (entry.summary.errors > 0 || entry.summary.invalid > 0)
    );
    if (firstProblematic) {
      setExpandedId(firstProblematic.id);
    }
  }, [entries]);
              ...
              <div className="space-y-1">
                ...
                {entry.summary && (
                  <div className="flex flex-wrap gap-2 text-xs">
                    {summaryDescriptors.map(({ key, label, color }) => {
                      const value = entry.summary?.[key] ?? 0;
                      if (value === 0) return null;
                      if (key === 'total' && (!entry.broadcast || value <= 1)) {
                        return null;
                      }
                      if (!entry.broadcast && key === 'success' && entry.summary.total <= 1) {
                        return null;
                      }
                      return (
                        <span
                          key={key}
                          className={`inline-flex items-center gap-1 rounded px-2 py-1 font-medium ${color}`}
                        >
                          <span>{label}:</span>
                          <span>{value}</span>
                          {key === 'total' && entry.broadcast && entry.summary.success === entry.summary.total && (
