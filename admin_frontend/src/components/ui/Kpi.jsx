/**
 * Подготовка денежного значения к переносу.
 *
 ...
 * не влезает, браузер рвёт его в произвольном месте: «10 577 2 / 49 ₽» —
 * цифры все на месте, но число не прочитать. Поэтому неразрывные пробелы
 * между разрядами заменяем обычными.
 *
 * А вот пробел перед знаком валюты возвращаем неразрывным: сумма должна
 * читаться как один объект, «10 577 249 / ₽» выглядит как ошибка вёрстки.
 * Так единственная возможная точка переноса — граница разрядов, и знак
 * валюты всегда остаётся при последнем разряде.
 *
 * Это страховка на крайний случай: размер шрифта (см. --kpi-len ниже)
 * подобран так, что обычные суммы вообще не переносятся.
 */
...
export default function Kpi({
  ...
  const len = typeof display === 'string' || typeof display === 'number'
