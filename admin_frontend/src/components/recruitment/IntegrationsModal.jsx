...
/** Часы звонков — окно, в котором «Прозвон» вообще предлагает кандидатов.
 *
 * Отдельно от часов общения намеренно: бот пишет в чат с девяти, а звонить
 * человеку в девять уже неловко. Оба окна стоят рядом именно чтобы разница
 * была видна, а не выяснялась опытным путём.
 */
function CallHoursPanel() {
  ...
  const load = useCallback(async () => {
    try {
      const res = await api.get('/recruitment/call-hours');
      setForm({ enabled: res.data.enabled, days: res.data.days, start: res.data.start, end: res.data.end });
      setStatus({ within_now: res.data.within_now, next_window_start: res.data.next_window_start });
    } catch (e) {
      setError(e.response?.data?.detail || e.message);
  ...
  async function save(next) {
    setBusy(true); setError(''); setSaved(false);
    try {
      const res = await api.put('/recruitment/call-hours', next);
      setForm({ enabled: res.data.enabled, days: res.data.days, start: res.data.start, end: res.data.end });
      setStatus({ within_now: res.data.within_now, next_window_start: res.data.next_window_start });
      setSaved(true);
      setTimeout(() => setSaved(false), 2500);
  ...
  return (
    <div className="rounded-xl border border-[color:var(--color-border)] p-3 space-y-3">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-sm font-semibold">Часы звонков</p>
          <p className="text-xs text-[color:var(--color-muted-foreground)] mt-0.5">
            Окно режима «Прозвон». Вне его очередь никого не предлагает, а назначенные
            звонки сдвигаются на начало ближайшего окна — назначить звонок на полночь
            нельзя даже вручную.
          </p>
        </div>
        <label className="flex items-center gap-1.5 text-xs flex-shrink-0 cursor-pointer">
      ...
      {form.enabled && (
        <>
          ...
          <p className={`text-xs ${status?.within_now ? 'text-emerald-600' : 'text-amber-600'}`}>
            {status?.within_now
              ? '● Сейчас можно звонить — очередь работает.'
              : `● Сейчас звонить рано${
                  status?.next_window_start ? `, окно откроется ${fmtNext(status.next_window_start)}` : ''
                }.`}
          </p>
        </>
      )}
