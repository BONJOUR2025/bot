...
/** На чём собрана сводка. Пустая строка, когда были оба источника: это
 *  обычный случай, и подписывать его нечем.
 *
 *  Оценка по одной анкете — не то же самое, что оценка после опроса, и
 *  без пометки уверенный балл по человеку, который не сказал ни слова,
 *  читается так же, как балл по прошедшему опрос.
 */
export function basisNote(profile) {
  return {
    resume: 'опрос не пройден — оценка по анкете',
    answers: 'анкеты с площадки нет — оценка по ответам',
    none: 'ни анкеты, ни ответов',
  }[profile?.basis] || '';
}
