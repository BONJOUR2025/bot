...
class MdmDevicesWatch(BaseModel):
    """Ответ живого обновления панели.

    Тот же приём, что у телефона: запрос висит, пока в парке ничего не
    менялось, и возвращается сразу, как только что-то произошло — снимок с
    камеры, ответ на команду, свежая телеметрия. Список телефонов кладём
    только когда есть что показать: на пустом ожидании гонять его незачем.
    """

    marker: str
    changed: bool
    devices: list[MdmDevice] = Field(default_factory=list)
