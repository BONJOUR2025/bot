...
class VacationRepository:
    def __init__(self, file_path: Optional[str] = None) -> None:
        ...
        self._counter = max((int(v.get("id", 0)) for v in self._data if str(v.get("id")).isdigit()), default=0)
    ...
    def create(self, data: Dict[str, Any]) -> Dict[str, Any]:
        if "id" not in data or any(str(v.get("id")) == str(data["id"]) for v in self._data):
            data["id"] = self._generate_id()
        self._data.append(data)
        self._save()
        return data

    def update(self, vac_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
