...
class AdjustmentRepository:
    def __init__(self, file_path: Optional[str] = None) -> None:
        ...
        self._counter = max((int(item.get("id", 0)) for item in self._data if str(item.get("id")).isdigit()), default=0)
    ...
    def delete(self, adj_id: str) -> None:
        self._data = [it for it in self._data if str(it.get('id')) != str(adj_id)]
        self._save()
