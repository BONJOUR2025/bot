...
DEFAULT_CATEGORIES = ["Связь", "Аренда", "ПО", "Коммунальные", "Налоги", "Страхование", "Прочее"]
...
class PaymentCalendarRepository:
    ...
    def create_category(self, name: str) -> Dict:
        with self._session() as db:
            row = PaymentCategory(name=name.strip(), sort_order=db.query(PaymentCategory).count())
            db.add(row)
            db.commit()
            db.refresh(row)
            return row.to_dict()
