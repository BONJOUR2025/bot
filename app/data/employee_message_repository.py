...
from app.config import EMPLOYEE_MESSAGES_FILE
from app.utils.logger import log


class EmployeeMessageRepository:
    def __init__(self, file_path: Optional[str] = None) -> None:
        self._file = file_path or EMPLOYEE_MESSAGES_FILE
    ...
    def update(self, message_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        for item in self._data:
            if str(item.get("id")) == str(message_id):
                item.update(updates)
                self._save()
