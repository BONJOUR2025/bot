...
class MessageRepository:
    def __init__(self, path: str | Path = "messages.json") -> None:
        ...
        self._counter = max((int(m.get("id", 0)) for m in self._data if str(m.get("id", "")).isdigit()), default=0)
    ...
    def list(self) -> List[Dict[str, Any]]:
        return sorted(self._data, key=lambda x: x.get("timestamp", ""), reverse=True)
