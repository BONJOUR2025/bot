...
def _log_tail(limit: int = 40) -> list[str]:
    r"""Последние строки лога бота.

    Строки до 29.08.2026 битые, и починить их при чтении нельзя: бот отдавал
    stderr в cp1251, pm2 читал поток как UTF-8, не мог его декодировать и
    подменял каждый русский символ на U+FFFD — в файле лежит уже замена, а не
    исходный байт. Исправлено в источнике (run.py бота переведён на UTF-8),
    но старые строки так и останутся «ромбиками».

    Экранированные последовательности (\uXXXX) — оттуда же: то, что cp1251
    закодировать не мог (рамки, эмодзи), Python отдавал escape-ом. Ниже они
    разворачиваются обратно, иначе в интерфейсе была бы каша из \uXXXX.
    Новые строки приходят готовым UTF-8, и на них замена просто не находит
    что менять.
    """
    try:
