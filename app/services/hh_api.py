...
#
# Важно не называть одну причину там, где их несколько: текст ошибки —
# это инструкция, куда идти разбираться, и неверная стоит часов поиска не там.
_HH_ERROR_HINTS = {
    # 04.09.2026: первые 15 сообщений рассылки ушли, следующие 9 подряд
    # получили 403 disabled_by_employer, а через час те же самые отклики
    # спокойно приняли сообщение. Вакансия при этом была активна
    # (archived=False), а старый текст утверждал обратное и отправлял
    # искать проблему в архиве. Поэтому лимит стоит первым: если сбой
    # пришёл посреди рассылки, это почти всегда он.
    "disabled_by_employer": (
        "hh.ru отказал в отправке по этому отклику (disabled_by_employer). "
        "Возможные причины, по убыванию вероятности: "
        "1) исчерпан лимит hh на сообщения работодателя — так бывает при рассылке "
        "подряд, и через некоторое время отпускает само; "
        "2) у тарифа нет доступа к переписке по этому отклику; "
        "3) вакансия архивирована или закрыта. "
