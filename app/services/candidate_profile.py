...
Сначала выбери полосу по самому сильному подходящему опыту. Опыт,
названный кандидатом в ответах, — такое же свидетельство, как строка в
анкете: сказанное «ремонтировал сумки» считается наравне с записью о
месте работы.
- 80-100 — профильный опыт от двух лет;
- 60-79 — профильный опыт меньше двух лет ИЛИ смежный опыт плюс
  готовность учиться;
- 40-59 — опыт работы есть, но далёкий от профиля;
- 20-39 — подходящего опыта нет, но человек готов учиться;
- 0-19 — человек не хочет эту работу.
Что считать профильным и смежным именно для этой вакансии — в блоке
КРИТЕРИИ ОЦЕНКИ. Если блока нет, выведи профиль из текста вакансии сам.
Не требуй, чтобы название прошлой должности совпадало с названием
вакансии: смысл работы важнее формулировки в резюме.
...
означает человека без профильного опыта, а не сапожника — кем бы он себя
ни назвал.
...
def format_vacancy(vacancy) -> str:
    ...
    # Что для ЭТОЙ вакансии профильный опыт, а что смежный. Раньше это
    # стояло в общем промпте словами «работа руками с кожей» — и любой
    # администратор-продавец получал бы полосу «подходящего опыта нет»
    # независимо от того, сколько лет он работал с клиентами.
    criteria = str(getattr(vacancy, "extra_instructions", "") or "").strip()
    if criteria:
        parts.append("КРИТЕРИИ ОЦЕНКИ:\n" + criteria[:2000])
    return "\n\n".join(parts)
