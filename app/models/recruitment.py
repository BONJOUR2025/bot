...
class Candidate(Base):
    ...
    #
    # ВНИМАНИЕ: хранится в ЛОКАЛЬНОМ времени, в отличие от остальных
    # datetime-полей этой модели (они UTC, datetime.utcnow). Так сделано
    # потому, что значение приходит из окна звонков, заданного локальными
    # часами, и сравнивается с локальным «сейчас»: конвертация туда-обратно
    # на каждом чтении добавила бы ровно тот класс ошибок, ради которого
    # call_queue.called_today и приводит UTC к локали в одном месте.
