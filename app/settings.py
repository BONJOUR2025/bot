...
class Settings(BaseSettings):
    ...
    # Как часто телефон присылает полный отчёт (телеметрия, список приложений,
    # политика), секунды. Читается живьём из config.json — менять частоту нужно
    # без пересборки APK И без деплоя.
    #
    # Раньше от этого числа зависела и задержка команд, поэтому оно держалось
    # низким (120) ценой батареи. С длинным опросом команды приходят за секунды
    # сами по себе, и частый тяжёлый отчёт стал лишней тратой — отсюда 600.
    mdm_command_poll_seconds: int = Field(600, validation_alias="MDM_COMMAND_POLL_SECONDS")

    # Сколько сервер держит запрос длинного опроса, секунды. Телефон висит на
    # одном открытом запросе, и команда уходит в него сразу, как появится —
    # отсюда отклик в секунды вместо интервала опроса. Держать дольше 30 с
    # рискованно: туннели и мобильные операторы рвут долгие простаивающие
    # соединения, а разрыв здесь стоит лишнего переподключения.
    mdm_long_poll_seconds: int = Field(25, validation_alias="MDM_LONG_POLL_SECONDS")
    ...
    # Снимки с камеры телефонов. В корне рабочей директории, не в static/:
    # это кадры с камеры, раздавать их без авторизации нельзя, поэтому и
    ...
    # артефактов сборки — а это единственное, что мешало обновлять парк
    # без участия человека.
    mdm_upload_token: str = Field("", validation_alias="MDM_UPLOAD_TOKEN")
