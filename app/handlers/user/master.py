...
DISCLAIMER = (
    "ℹ️ Цифры предварительные, посчитаны по сканам. "
    "Итоговая выплата может отличаться — премии, штрафы и корректировки "
    "считаются отдельно."
)
...
def format_earnings(master, report: dict) -> str:
    ...
    to_pay = report["to_pay"]
    if to_pay < 0:
        # Минус на экране у человека читается как долг и пугает, хотя
        # означает всего лишь «аванс уже перекрыл начисленное».
        lines.append("<b>К выплате сейчас: 0 ₽</b>")
        lines.append(
            f"Авансом получено на {money(-to_pay)} больше начисленного — "
            "разница учтётся при расчёте зарплаты."
        )
    else:
        lines.append(f"<b>К выплате сейчас: {money(to_pay)}</b>")

    if report["warnings_count"]:
        lines.append("")
        lines.append(
            f"⚠️ Услуг с замечаниями: {report['warnings_count']} — "
