...
class TestChatPolza:
    ...
    def test_successful_call_records_usage(self, monkeypatch):
        def fake_post(url, json=None, headers=None, timeout=None):
            return httpx.Response(
                200,
                json={
                    ...
                    "usage": {"prompt_tokens": 13, "completion_tokens": 4,
                              "total_tokens": 17, "cost_rub": 0.00086667},
                },
                request=httpx.Request("POST", url),
            )
        ...
        monkeypatch.setattr("app.services.llm_usage_service.record_usage", fake_record_usage)
        llm.chat(self.BASE, [{"role": "user", "content": "hi"}])
    ...
    def test_usage_recording_failure_does_not_break_the_reply(self, monkeypatch):
        """A logging bug must never take down an actual candidate reply."""
        def fake_post(url, json=None, headers=None, timeout=None):
            return httpx.Response(
                200, json={"choices": [{"message": {"content": "ok"}}]},
                request=httpx.Request("POST", url),
        ...
        monkeypatch.setattr(httpx, "post", fake_post)
        monkeypatch.setattr("app.services.llm_usage_service.record_usage", boom)
