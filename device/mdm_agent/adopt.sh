...
# Git Bash подменяет пути вида /sdcard/... на виндовые ДО запуска adb,
# поэтому переменную экспортируем в окружение скрипта, а не префиксом к
# команде: префикс доходит до adb, но подмена уже случилась в шелле, и
# `adb push /sdcard/...` молча уезжает в C:/Program Files/Git/sdcard.
...
if [ -n "$accounts" ]; then
  echo "$accounts" | sed 's/^/  · /'
  echo
  echo "  Удалите все. Записи приложений (WhatsApp, VK) из «Настройки → Аккаунты"
  echo "  → Удалить» уходят, но приложение заводит их заново при запуске — на"
  echo "  первом Redmi мы на этом потеряли попытку. Порядок надёжный:"
  echo
  echo "    adb shell pm disable-user --user 0 <пакет>   # чтобы не вернул"
  echo "    убрать аккаунт в Настройках"
  echo "    bash adopt.sh"
  echo "    adb shell pm enable <пакет>                  # вернуть после"
  echo
  echo "  Отключение приложения данные НЕ трогает: у салонного телефона так"
  echo "  уцелели 33 ГБ переписки WhatsApp."
  echo
  echo "  Если приложение удалялось целиком — НУЖНА ПЕРЕЗАГРУЗКА: строка"
...
case "$(adb_ shell getprop ro.product.brand 2>/dev/null | tr -d '\r' | tr 'A-Z' 'a-z')" in
  xiaomi|redmi|poco)
    say "Сканер MIUI"
